# QGIS Project Snapshot — user guide / instrukcja 1.0.2

## English — installation and export

Requires QGIS 3.40–3.x (Qt5) or QGIS 4.x (Qt6), with GDAL 3.7 or newer
...
1. Choose **Plugins → Manage and Install Plugins → Install from ZIP** and select
   `qgis-project-snapshot-1.0.2.zip`. Restart QGIS when upgrading.
...
Some early development archives lack source/style compatibility information;
the plugin reports this limitation. Official 1.0.0 stores that information.

Map progress is saved during downloads. After an unexpected QGIS exit,
...
Tile continuation requires a saved tile record. Early development archives
without one retain completed layers but restart an unfinished map.
...
`diagnostic.jsonl` and `manifest.json` are in `diagnostics/`.
...
The main folder contains the project, report, `data/` and required
...
Wymagane: QGIS 3.40–3.x (Qt5) lub QGIS 4.x (Qt6), z GDAL co najmniej 3.7
...
2. Wskaż `qgis-project-snapshot-1.0.2.zip`. Po aktualizacji uruchom ponownie QGIS.
...
Niektóre wczesne archiwa rozwojowe nie mają informacji o zgodności źródeł
i stylów; wtyczka zgłasza to ograniczenie. Oficjalne 1.0.0 zapisuje te informacje.

Postęp map jest zapisywany podczas pobierania. Po nieoczekiwanym zamknięciu
...
Kontynuacja kafelków wymaga zapisanego rejestru postępu. Wczesne archiwa
rozwojowe bez rejestru zachowują ukończone warstwy, ale pobierają przerwaną mapę
od początku.
...
opis archiwum. W folderze głównym pozostają projekt `.qgz`, `raport.html`,
...
`diagnostyka.jsonl` i `manifest.json` są w podfolderze `diagnostyka/`.
