...
if "QGIS_CUSTOM_CONFIG_PATH" not in os.environ:
    TEST_PROFILE = TemporaryDirectory(prefix="snapshot-raster-profile-")
...
class RasterTests(unittest.TestCase):
    def setUp(self):
        ...
        self.area = QgsGeometry.fromWkt(
            (
                "POLYGON((500000 500000,500512 500000,500512 "
                "500512,500000 500512,500000 500000),(500180 "
                "500180,500180 500330,500330 500330,500330 500180,500180 "
                "500180))"
            )
        )
        self.layer = QgsVectorLayer(
            "Polygon?crs=EPSG:2180", "Półprzezroczysta", "memory"
        )
        feature = QgsFeature()
        feature.setGeometry(
            QgsGeometry.fromRect(QgsRectangle(499900, 499900, 500600, 500600))
        )
        self.layer.dataProvider().addFeatures([feature])
        self.layer.updateExtents()
        self.layer.renderer().setSymbol(
            QgsFillSymbol.createSimple(
                {"color": "30,100,170,128", "outline_style": "no"}
            )
        )
    ...
    def render(self, **kwargs):
        return write_rendered_raster(
            ...
            self.database,
            "map",
            zoom_levels(self.project, self.area, self.crs, 16, 17),
            kwargs.get("cancelled", lambda: False),
            kwargs.get("progress", lambda text: None),
        )
    ...
    def test_rgba_mask_holes_compression_and_native_epsg(self):
        ...
        with closing(sqlite3.connect(self.database)) as db:
            self.assertEqual(
                db.execute("SELECT srs_id FROM gpkg_contents").fetchone()[0], 2180
            )
            self.assertEqual(
                [
                    r[0]
                    for r in db.execute(
                        "SELECT zoom_level FROM gpkg_tile_matrix ORDER BY zoom_level"
                    )
                ],
                [16, 17],
            )
        ...
        for (payload,) in tiles:
            ...
            self.assertEqual(
                payload[index + 9] >> 6, 3
            )  # zlib maximum-compression FLEVEL.
        ds = gdal.OpenEx(
            str(self.database),
            gdal.OF_RASTER,
            open_options=["TABLE=map", "ZOOM_LEVEL=17"],
        )
        transform = ds.GetGeoTransform()

        def alpha_at(x, y):
            column, row = (
                int((x - transform[0]) / transform[1]),
                int((y - transform[3]) / transform[5]),
            )
    ...
    def test_zoom_scales_and_small_area_have_every_requested_level(self):
        ...
        for previous, current in zip(levels, levels[1:]):
            ...
            self.assertAlmostEqual(
                current["scale"], current["resolution"] * 96 / 0.0254, places=5
            )
        area = QgsGeometry.fromRect(QgsRectangle(500000, 500000, 500001, 500001))
        record = write_rendered_raster(
            self.layer,
            self.project,
            area,
            self.crs,
    ...
    def test_independent_zoom_images_not_downsampled(self):
        ...
        for zoom, red in ((16, 255), (17, 0)):
            ds = gdal.OpenEx(
                str(self.database),
                gdal.OF_RASTER,
                open_options=["TABLE=map", f"ZOOM_LEVEL={zoom}"],
            )
    ...
    def test_empty_result_is_reported_and_transparent_tiles_not_stored(self):
        ...
        with patch(
            "mbtiles_batch_exporter.raster_archive._render_image", side_effect=empty
        ):
            record = self.render()
    ...
    def test_retries_then_smaller_fragments_and_partial_failure(self):
        ...
        with patch(
            "mbtiles_batch_exporter.raster_archive._render_image", side_effect=smaller
        ):
            record = self.render()
        self.assertEqual(record["status"], "saved")
        ...
        with patch(
            ...
            side_effect=RuntimeError("Test: brak usługi"),
        ):
            record = self.render()
    ...
    def test_edit_buffer_is_present_in_fallback_rendering(self):
        ...
        feature = QgsFeature()
        feature.setGeometry(
            QgsGeometry.fromRect(QgsRectangle(500400, 500400, 500500, 500500))
        )
        ...
        def check_edits(layer, *args):
            self.assertEqual(
                {f.geometry().asWkt() for f in layer.getFeatures()}, expected
            )
            return _render_image(layer, *args)

        with patch(
            "mbtiles_batch_exporter.raster_archive._render_image",
            side_effect=check_edits,
        ):
            self.render()
    ...
    def test_cancel_removes_unfinished_raster_but_keeps_completed_vector(self):
        ...
        with closing(sqlite3.connect(result / "dane.gpkg")) as db:
            ...
            self.assertEqual(
                db.execute("SELECT count(*) FROM gpkg_tile_matrix").fetchone()[0], 0
            )
    ...
    def test_numerical_raster_keeps_values_and_style_after_move(self):
        ...
        self.assertEqual(
            struct.unpack("H", ds.GetRasterBand(1).ReadRaster(100, 100, 1, 1))[0], 40000
        )
        self.assertEqual(
            ds.GetRasterBand(1).GetMaskBand().ReadRaster(250, 250, 1, 1)[0], 0
        )
        ds = None
    ...
    def start_server(self):
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_GET(self):
                ...
                if parameters.get("REQUEST", "").lower() == "getcapabilities":
                    ...
                    xlink:href="http://{self.headers["Host"]}/wms"/></Get></HTTP></DCPType></GetMap>
                    ...
                    <BoundingBox CRS="EPSG:2180" minx="499000" miny="499000"
                    maxx="520000" maxy="520000"/></Layer>
                    ...
                    with self.server.lock:
                        self.server.active -= 1
                    width, height = (
                        int(parameters.get("WIDTH", 256)),
                        int(parameters.get("HEIGHT", 256)),
                    )
                    with self.server.lock:
                    ...
                    if status:
                        ...
                        self.send_header(
                            "Retry-After", getattr(self.server, "retry_after", "0")
                        )
        ...
        try:
            ...
            raise unittest.SkipTest(
                "Loopback HTTP wymaga zezwolenia sieciowego w tym środowisku."
            )
    ...
    def capture(self, workers, per_server_limit=2):
        ...
        if workers > 1:
            self.assertTrue(
                any(
                    r["phase"] == "active"
                    and ("fragment" in r["message"] or "zoom" in r["message"])
                    for r in activities
                ),
                activities,
            )
            self.assertEqual(
                manifest["parallel"]["completed_in_workers"], 2, manifest["layers"]
            )
            self.assertEqual(
                len({r["worker_pid"] for r in manifest["layers"] if "worker_pid" in r}),
                2,
            )
        ...
        if os.environ.get("QGIS_SNAPSHOT_LANGUAGE") == "en":
            ...
            self.assertTrue(
                any("tile" in row["message"] for row in activities), activities
            )
