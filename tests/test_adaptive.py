...
from mbtiles_batch_exporter.adaptive import HostPolicy, WorkerGate, HostDeferred, DownloadError, retry_after, write_state
...
class PolicyTests(unittest.TestCase):
    ...
    def test_expired_coordinator_permission_fails_closed(self):
        with TemporaryDirectory() as temporary:
            folder = Path(temporary)
            gate = WorkerGate(folder, lambda: False)
            write_state(folder / 'control.json', {'version': 1, 'allowed': True, 'expires': 0})
            with patch('mbtiles_batch_exporter.adaptive.time.monotonic', side_effect=[0, 11, 11]):
                with self.assertRaises(HostDeferred):
                    gate.before()
    ...
    def test_lowered_permission_blocks_next_operation_and_can_be_cancelled(self):
        with TemporaryDirectory() as temporary:
            folder = Path(temporary)
            cancel = [False]
            gate = WorkerGate(folder, lambda: cancel[0], lambda: cancel.__setitem__(0, True))
            command = {'version': 1, 'allowed': True, 'expires': time.monotonic() + 10, 'generation': 0, 'ack': 0}
    ...
    def test_coordinator_resamples_ram_and_reports_pressure(self):
        ...
        with (patch('mbtiles_batch_exporter.parallel_archive.time.monotonic', side_effect=[20, 40]),
              patch('mbtiles_batch_exporter.parallel_archive.available_memory', side_effect=[1024**3, 3 * 1024**3]) as memory):
            coordinator._coordinate()
        self.assertEqual(memory.call_count, 2)
        self.assertEqual([r['memory_ok'] for r in coordinator.memory_history], [False, True])
...
class RepairTests(unittest.TestCase):
    ...
    def test_only_failed_tiles_are_repaired_including_empty_successes(self):
        ...
        def render(layer, project, bounds, width, height, cancelled, progress):
            ...
            return fixtures._render_image(layer, project, bounds, width, height, cancelled, progress)
        with patch('mbtiles_batch_exporter.raster_archive._render_image', side_effect=render):
            result = fixtures.write_rendered_raster(self.layer, self.project, self.area, self.crs,
                self.database, 'map', fixtures.zoom_levels(self.project, self.area, self.crs, 17, 17),
                lambda: False, lambda _: None, gate=self.gate())
        ...
        with closing(sqlite3.connect(self.folder / 'map.tiles.sqlite')) as db:
            self.assertGreater(db.execute('SELECT count(*) FROM tiles WHERE status="empty"').fetchone()[0], 0)
            self.assertEqual(db.execute('SELECT max(attempts) FROM tiles').fetchone()[0], 2)
    ...
    def test_permanent_error_is_not_retried_and_remains_missing(self):
        ...
        with patch('mbtiles_batch_exporter.raster_archive._render_image', side_effect=fail):
            result = fixtures.write_rendered_raster(self.layer, self.project, self.area, self.crs,
                self.database, 'map', fixtures.zoom_levels(self.project, self.area, self.crs, 17, 17),
                lambda: False, lambda _: None, gate=self.gate())
    ...
    def test_repair_budget_is_three_total_attempts_per_tile(self):
        ...
        with closing(sqlite3.connect(self.folder / 'map.tiles.sqlite')) as db:
            self.assertEqual(db.execute('SELECT max(attempts) FROM tiles').fetchone()[0], 3)
    ...
    def test_cancel_during_repair_does_not_repeat_successful_tiles(self):
        ...
        with patch('mbtiles_batch_exporter.raster_archive._render_image', side_effect=render):
            with self.assertRaises(InterruptedError):
                fixtures.write_rendered_raster(self.layer, self.project, self.area, self.crs,
                    self.database, 'map', fixtures.zoom_levels(self.project, self.area, self.crs, 17, 17),
                    lambda: False, lambda _: None, gate=gate)
...
class AdaptiveWmsTests(unittest.TestCase):
    ...
    def add_map(self, host='127.0.0.1', name='Adaptive WMS'):
        ...
        for key, value in {'url': f'http://{host}:{self.server.server_port}/wms', 'layers': 'map',
    ...
    def capture(self, layers, **kwargs):
        ...
        with patch('mbtiles_batch_exporter.archive.detect_resources', return_value={'cpu': 2, 'memory': 8 * 1024**3, 'online': True}):
            result = fixtures.create_archive(self.project, [l.id() for l in layers], self.area,
                self.crs, self.folder, zoom_min=17, zoom_max=17, adaptive=True, **kwargs)
    ...
    def test_rate_limit_repairs_same_map_and_records_recovery(self):
        ...
        rows = []
        result, manifest = self.capture([layer], server_activity=lambda values: rows.extend(values))
    ...
    def test_long_retry_after_defers_one_host_but_another_finishes(self):
        ...
        records = [r for r in manifest['layers'] if r['id'] in (first.id(), second.id())]
        ...
        self.assertTrue(any('retry_after_exceeds_300s' in [h['reason'] for h in p['history']]
    ...
    def test_cancel_during_cooldown_preserves_completed_vector(self):
        ...
        def observe(rows):
            if any(r['state'] == 'cooldown' for r in rows):
                cancel[0] = True
        _, manifest = self.capture([self.layer, layer], server_activity=observe, cancelled=lambda: cancel[0])
    ...
    def test_worker_crash_never_falls_back_outside_host_control(self):
        layer = self.add_map()
        with patch('mbtiles_batch_exporter.parallel_archive.RasterWorkers._run', side_effect=RuntimeError('crash')):
    ...
    def test_http_date_retry_after_and_503_recovery(self):
        ...
        self.server.retry_after = format_datetime(datetime.now(timezone.utc) + timedelta(seconds=4), usegmt=True)
        _, manifest = self.capture([layer])
        record = next(r for r in manifest['layers'] if r['id'] == layer.id())
        self.assertEqual(record['status'], 'saved', record)
        history = manifest['adaptive']['hosts'][0]['history']
    ...
    def test_main_qgis_map_uses_same_host_controller(self):
        ...
        with patch.object(layer, 'source', return_value=layer.source() + '&authcfg=test'):
            _, manifest = self.capture([layer])
        record = next(r for r in manifest['layers'] if r['id'] == layer.id())
        self.assertEqual(record['status'], 'saved', record)
        self.assertNotIn('worker_pid', record)
        self.assertEqual(record['raster']['repaired'], 1)
        self.assertIn('429', [h['reason'] for h in manifest['adaptive']['hosts'][0]['history']])
