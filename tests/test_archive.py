...
from qgis.core import (
    QgsApplication, QgsCoordinateReferenceSystem, QgsFeature,
    QgsGeometry, QgsProject, QgsRasterLayer, QgsVectorFileWriter, QgsVectorLayer,
)
...
class ArchiveTests(unittest.TestCase):
    ...
    def add_points(self, name, coordinates):
        layer = QgsVectorLayer('Point?crs=EPSG:2180&field=fid:integer&field=opis:string', name, 'memory')
        features = []
    ...
    def archive(self, selected=None, **kwargs):
        return create_archive(
            self.project, selected if selected is not None else self.project.mapLayers().keys(),
            self.area, self.crs, self.folder, **kwargs,
        )
    ...
    def test_vectors_keep_attributes_edits_styles_tree_and_source_state(self):
        ...
        self.assertTrue(self.layer.changeAttributeValue(feature.id(), 1, 'niezapisana zmiana'))
        self.assertTrue(self.layer.changeGeometry(original_features[1].id(), QgsGeometry.fromWkt('POINT(8 8)')))
        ...
        try:
            ...
            self.assertIn('niezapisana zmiana', [f['opis'] for f in local.getFeatures()])
            self.assertIn('niezapisany nowy obiekt', [f['opis'] for f in local.getFeatures()])
            self.assertEqual({f.geometry().asWkt() for f in local.getFeatures()},
                             {'Point (5 5)', 'Point (7 7)', 'Point (8 8)'})
            self.assertEqual(local.renderer().symbol().color().name(), '#123456')
            self.assertEqual(local.attributeAlias(local.fields().indexFromName('opis')), 'Opis obiektu')
            self.assertFalse(copy.layerTreeRoot().findGroup('Wyłączona grupa').itemVisibilityChecked())
            self.assertEqual([l.id() for l in copy.layerTreeRoot().customLayerOrder()], [second.id(), self.layer.id()])
        finally:
            copy.clear()
    ...
    def test_filters_materialized_and_whole_geometry_kept(self):
        ...
        local = QgsVectorLayer(f'{result}/dane.gpkg|layername={row["table"]}', 'local', 'ogr')
        self.assertEqual(next(local.getFeatures()).geometry().asWkt(), feature.geometry().asWkt())

    def test_excluded_unsupported_and_invalid_layers_reported_without_remote_sources(self):
        other = self.add_points('Pominięta', [(3, 3)])
        invalid = QgsVectorLayer(str(self.folder / 'missing.gpkg'), 'Brak źródła', 'ogr')
    ...
    def test_polygon_selection_and_invalid_area(self):
        ...
        with self.assertRaises(ValueError):
            create_archive(self.project, [self.layer.id()], QgsGeometry(), self.crs, self.folder)
        with self.assertRaises(ValueError):
            self.archive([])
    ...
    def test_cancel_during_layer_removes_incomplete_table(self):
        ...
        with closing(sqlite3.connect(result / 'dane.gpkg')) as database:
            self.assertEqual(database.execute('SELECT count(*) FROM gpkg_contents').fetchone()[0], 0)
    ...
    def test_zero_features_is_valid_not_a_download_error(self):
        ...
        record = next(r for r in self.manifest(result)['layers'] if r['id'] == empty.id())
    ...
    def test_coordinate_transform_to_layer_crs(self):
        ...
        area = QgsGeometry.fromWkt('POLYGON((18.9 51.9,19.1 51.9,19.1 52.1,18.9 52.1,18.9 51.9))')
        ...
        record = next(r for r in self.manifest(result)['layers'] if r['id'] == layer.id())
