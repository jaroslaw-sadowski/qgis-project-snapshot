...
def benchmark(output):
    ...
    try:
        ...
        for index in range(4):
            ...
            for key, value in {
                "url": f"http://{host}:{server.server_port}/wms",
                "layers": "map",
                "styles": "",
                "format": "image/png",
                "crs": "EPSG:2180",
            ...
            layer = QgsRasterLayer(
                bytes(uri.encodedUri()).decode(), f"Mapa {index + 1}", "wms"
            )
        ...
        scenarios = {
            ...
            "corridor": QgsGeometry.fromWkt(
                "LINESTRING(500000 500000,507071 507071)"
            ).buffer(75, 4),
        }
        ...
        with TemporaryDirectory(prefix="qgis-benchmark-") as temporary:
            ...
            for scenario, area in scenarios.items():
                for workers in (1, 2, 4):
                    ...
                    folder.mkdir()
                    before_requests = sum(
                        len(server.requests) for server in (first.server, second.server)
                    )
                    for server in (first.server, second.server):
                        server.peak = 0
                    stop = Event()
                    ...
                    monitor = Thread(
                        target=measure_memory_and_disk,
                        args=(folder, stop, samples),
                        daemon=True,
                    )
                    monitor.start()
                    ...
                    if checksum != expected[scenario]:
                        raise RuntimeError(
                            (
                                "Równoległe warianty mają inne obrazy niż wariant "
                                "pojedynczy."
                            )
                        )
                    row = {
                        ...
                        **samples,
                        ...
                        "requests": sum(
                            len(s.requests) for s in (first.server, second.server)
                        )
                        - before_requests,
                        "server_peaks": [s.peak for s in (first.server, second.server)],
                        "identical_png": True,
                        "worker_processes": len(
                            {r["worker_pid"] for r in records if "worker_pid" in r}
                        ),
                    }
                    rows.append(row)
            ...
            for result, area in archived:
                ...
                try:
                    if not offline.read(str(next(moved.glob("*.qgz")))):
                        raise RuntimeError(
                            "Nie można otworzyć archiwum po przeniesieniu."
                        )
                    for layer in offline.mapLayers().values():
                        ...
                        if image.isNull() or not any(
                            image.pixelColor(i, i).alpha() for i in range(512)
                        ):
                ...
                json.dumps(
                    {
                        ...
                        "rss_note": (
                            "RSS sum includes shared pages counted per process."
                        ),
                    },
                    indent=2,
                )
