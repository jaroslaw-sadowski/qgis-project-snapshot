...
class ProgressTests(unittest.TestCase):
    ...
    def test_real_export_shows_layer_progress_stages_and_result(self):
        dialog = self.dialog()
        try:
            with patch('mbtiles_batch_exporter.archive_dialog.QMessageBox.information',
    ...
    def test_failure_does_not_show_success_and_retains_diagnostic_log(self):
        dialog = self.dialog()
        try:
            with (patch('mbtiles_batch_exporter.archive_dialog.create_archive', side_effect=RuntimeError('Test: brak miejsca')),
    ...
    def test_cancel_in_dialog_keeps_finished_layer_and_reports_interruption(self):
        ...
        try:
            with (patch.object(dialog, '_layer_status', side_effect=status),
    ...
    def test_worker_messages_are_deduplicated_and_do_not_overwrite_final_status(self):
        dialog = self.dialog()
        try:
            row = {'id': self.layer.id(), 'phase': 'active', 'message': 'zoom 17, fragment 2'}
            ...
            dialog._layer_status({'id': self.layer.id(), 'status': 'failed', 'reason': 'Nie udało się pobrać.'}, 1, 1)
            dialog._worker_activity([row])
