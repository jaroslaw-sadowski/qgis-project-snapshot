...
class ProjectResources:
    ...
    def copy(self, value, owner, base=None):
        ...
        if re.match(r"^https?://", value, re.I):
            self.issue(
                owner, tr("Zasób internetowy wymaga ręcznego zapisania i sprawdzenia.")
            )
            return value
        if value.startswith("attachment:"):
            ...
            if not path.is_absolute():
                path = (
                    Path(base) if base else Path(self.project.homePath() or ".")
                ) / path
            if not path.is_file():
                ...
                for node in tree.iter():
                    ...
                    if node.tag in ("customwidgets", "resources") and len(node):
                        self.issue(
                            ...
                            tr(
                                (
                                    "Formularz używa dodatkowych komponentów Qt; "
                                    "wymaga sprawdzenia."
                                )
                            ),
                        )
                ...
                for feature in layer.getFeatures():
                    ...
                    if isinstance(value, str) and value:
                        ...
                        if copied != value and not layer.changeAttributeValue(
                            ...
                            raise RuntimeError(
                                tr("Nie udało się zapisać ścieżki załącznika.")
                            )
    ...
    def rewrite(self, root, records):
        ...
        if relations is not None:
            for relation in list(relations):
                if (
                    relation.get("referencingLayer") in vectors
                    and relation.get("referencedLayer") in vectors
                ):
                    valid_relations.add(relation.get("id"))
                    self.issue(
                        relation.get("name") or "Relacja",
                        tr(
                            (
                                "Relacja zachowana; obszar eksportu może pomijać "
                                "powiązane obiekty poza obszarem."
                            )
                        ),
                    )
                else:
                    ...
                    self.issue(
                        ...
                        tr(
                            (
                                "Usunięto relację do warstwy niezapisanej jako dane "
                                "wektorowe."
                            )
                        ),
                    )
        for element in root.findall("./projectlayers/maplayer"):
            ...
            for widget in element.findall(".//editWidget"):
                ...
                missing = (
                    widget.get("type") == "ValueRelation"
                    and options.get("Layer") not in vectors
                ) or (
                    widget.get("type") == "RelationReference"
                    and options.get("Relation") not in valid_relations
                )
                if missing:
                    ...
                    self.issue(
                        ...
                        tr(
                            (
                                "Pole formularza wskazywało brakującą relację; "
                                "pozostawiono odczyt wartości."
                            )
                        ),
                    )
            for parent in element.iter():
                for child in list(parent):
                    if (
                        child.tag == "attributeEditorRelation"
                        and child.get("relation") not in valid_relations
                    ):
                        parent.remove(child)
            ...
            if (element.findtext("editforminit") or "").strip():
                self.issue(
                    owner, tr("Kod formularza wymaga ręcznej kontroli zależności.")
                )
            if element.findall("./attributeactions/actionsetting"):
                self.issue(
                    ...
                    tr(
                        (
                            "Akcje warstwy mogą uruchamiać zewnętrzne zasoby; wymagają "
                            "kontroli."
                        )
                    ),
                )
        ...
        for node in root.iter():
            ...
            if (
                ...
                self.issue(
                    ...
                    tr(
                        (
                            "Projekt zawiera wyrażenia dynamiczne; ich zależności "
                            "wymagają kontroli."
                        )
                    ),
                )
            if node.tag == "LayoutItem" and (node.get("html") or node.get("url")):
                self.issue(
                    tr("Układ wydruku"),
                    tr("Element HTML wymaga kontroli zewnętrznych zasobów."),
                )
        if root.findall(".//Layout"):
            self.issue(
                ...
                tr(
                    (
                        "Układy i atlas wymagają odbioru wizualnego po ograniczeniu "
                        "danych do obszaru."
                    )
                ),
            )
        if root.findall("./polymorphicRelations/relation"):
            self.issue(
                "Relacje", tr("Relacje polimorficzne wymagają ręcznej kontroli.")
            )
    ...
    try:
        ...
        for record in records:
            ...
            if layer is None or not layer.isValid():
                failures.append(
                    tr("Nie można otworzyć lokalnej warstwy: {0}").format(
                        record["name"]
                    )
                )
        return failures
